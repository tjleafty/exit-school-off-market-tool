...
export default function CompanyDiscoveryPage() {
  ...
  const exportSelectedTXT = () => {
    ...
    // Header
    txtContent += '================================================================================\n'
    txtContent += '            Exit School Off-Market Tool - Company Search Results\n'
    txtContent += '================================================================================\n\n'
    
    // Report metadata
    txtContent += `Export Generated: ${new Date().toLocaleDateString('en-US')} ${new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}\n`
    txtContent += `Total Companies: ${selectedData.length}\n\n`
    
    txtContent += '================================================================================\n'
    txtContent += '                              COMPANY LISTINGS\n'
    txtContent += '================================================================================\n\n'

    // Process each company
    selectedData.forEach((company, index) => {
      txtContent += `[${index + 1}] ${company.name || 'N/A'}\n`
      txtContent += '─'.repeat(60) + '\n'
      
      // Basic Information
      if (company.address || company.city || company.state) {
        txtContent += '📍 Location:\n'
        if (company.address) txtContent += `   Address: ${company.address}\n`
        if (company.city || company.state) {
          const location = [company.city, company.state].filter(Boolean).join(', ')
          txtContent += `   Location: ${location}\n`
        }
        txtContent += '\n'
      }
      ...
      txtContent += `   Phone: ${company.phone || 'Not Available'}\n`
      txtContent += `   Website: ${company.website || 'Not Available'}\n\n`
      
      // Business Information
      txtContent += '🏢 Business Details:\n'
      txtContent += `   Industry: ${company.industry || 'Not Specified'}\n`
      if (company.rating) {
        txtContent += `   Google Rating: ${company.rating}/5.0 (${company.user_ratings_total || '0'} reviews)\n`
      }
      if (company.place_id) {
        txtContent += `   Google Maps: https://www.google.com/maps/place/?q=place_id:${company.place_id}\n`
      }
      txtContent += '\n'
      
      // Technical Information
      txtContent += '🔍 Discovery Information:\n'
      if (company.id) txtContent += `   Company ID: ${company.id}\n`
      if (company.place_id) txtContent += `   Google Place ID: ${company.place_id}\n`
    ...
    txtContent += '================================================================================\n'
    txtContent += `Generated by Exit School Off-Market Tool on ${new Date().toLocaleDateString('en-US')}\n`
    txtContent += `Total Companies Exported: ${selectedData.length}\n`
    ...
    const url = URL.createObjectURL(blob)
    link.setAttribute('href', url)
    link.setAttribute('download', `companies_export_${new Date().toISOString().split('T')[0]}.txt`)
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    
    alert(`Exported ${selectedData.length} companies to TXT successfully!`)
  }
            ...
            <div className="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between items-center">
              ...
              {selectedCompanies.length > 0 && (
                <div className="flex space-x-2">
                  ...
                  >
                    📊 Export CSV ({selectedCompanies.length})
                  </button>
                  <button 
                    onClick={exportSelectedTXT}
                    className="px-3 py-1 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700"
                  >
                    📄 Export TXT ({selectedCompanies.length})
                  </button>
                  <button 
                    onClick={addToList}
